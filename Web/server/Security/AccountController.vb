...
Public Class AccountController
    ...
    <Authorize>
    Public Function GetAll() As IEnumerable
        Dim out = (From p In ApplicationDbContext.Create.Users
                  Order By p.FullName, p.Email).ToList
                  
        Return From p In out Select New With {.DisplayName = IIf(String.IsNullOrEmpty(p.FullName), p.Email, p.FullName & " &lt;" & p.Email & "&gt;"),
                                              .SelectedName = p.FullName & " <" & p.Email & ">",
                                   .Id = p.Id,
                                   .Username = p.UserName,
                                   .Email = p.Email,
                                   .FullName = p.FullName}
    ...
    '    Dim userInfo As User = Await UserManager.FindAsync(New UserLoginInfo(externalLogin.LoginProvider, externalLogin.ProviderKey))
    ...
    <AllowAnonymous>
    <Route("Register")>
    Public Async Function Register(model As RegisterBindingModel) As Task(Of Object)
        ...
        Dim loginResult = LoginUser(New LoginUserBindingModel() With {.Username = model.Email, .Password = model.Password})
    ...
    <AllowAnonymous>
    <Route("Login")>
    Public Async Function LoginUser(model As LoginUserBindingModel) As Task(Of HttpResponseMessage)

        ' Invoke the "token" OWIN service to perform the login: /api/token
        ' Ugly hack: I use a server-side HTTP POST because I cannot directly invoke the service (it is deeply hidden in the OAuthAuthorizationServerHandler class)
        Dim request = System.Web.HttpContext.Current.Request
        Dim tokenServiceUrl = request.Url.GetLeftPart(UriPartial.Authority) + request.ApplicationPath + TOKEN_SERVICE_URL

        Using client As New HttpClient()

            Dim requestParams = New List(Of KeyValuePair(Of String, String))
            requestParams.Add(New KeyValuePair(Of String, String)("grant_type", "password"))
            requestParams.Add(New KeyValuePair(Of String, String)("username", model.Username))
            requestParams.Add(New KeyValuePair(Of String, String)("password", model.Password))

            Dim requestParamsFormUrlEncoded As New FormUrlEncodedContent(requestParams)
            Dim tokenServiceResponse = Await client.PostAsync(tokenServiceUrl, requestParamsFormUrlEncoded)
            Dim responseString = Await tokenServiceResponse.Content.ReadAsStringAsync()
            Dim responseCode = tokenServiceResponse.StatusCode
            Dim responseMsg = New HttpResponseMessage(responseCode)

            responseMsg.Content = New StringContent(responseString, Encoding.UTF8, "application/json")
            Return responseMsg
    ...
    Public Function Update(user As User) As HttpResponseMessage

        Dim result As IdentityResult = UserManager.Update(user)
